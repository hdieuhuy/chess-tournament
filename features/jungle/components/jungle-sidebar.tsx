...
export function JungleSidebar({ isDarkMode, onReady }: { isDarkMode: boolean; onReady?: () => void }) {
  ...
  const canUndo = () => {
    if (isSpectator || history.length < 1) return false;
    ...
    if (isRedTurn !== requesterTurn) return false;

    return true;
  };

  const statusText = winner
    ? `Winner: ${winner === "r" ? (gameMode === "2v2" ? "Đội Đỏ" : player1Name) : (gameMode === "2v2" ? "Đội Xanh" : player2Name)}`
    : isGameStarted
      ? `Đang chơi - Lượt: ${gameMode === "2v2" 
        ? turnIndex === 0 ? player1Name : turnIndex === 1 ? player2Name : turnIndex === 2 ? player3Name : player4Name
        : isRedTurn ? player1Name : player2Name}`
      : "Đang chờ bắt đầu...";
    ...
    <div className={`flex flex-col h-full rounded-2xl border shadow-sm overflow-hidden transition-colors ${isDarkMode ? "bg-slate-800 border-slate-700" : "bg-white border-zinc-200"}`}>
      ...
      <div className={`flex flex-col p-4 border-b transition-colors ${isDarkMode ? "bg-slate-900/50 border-slate-700" : "bg-zinc-50 border-zinc-200"}`}>
        ...
        <div className="flex justify-between items-center w-full pt-2">
          <div className="flex items-center gap-1.5 sm:gap-2">
            {isGameStarted && !winner && !isSpectator && canUndo() && (
            ...
            {isGameStarted && !winner && !isSpectator && (
              <div className="relative group flex justify-center">
                <button
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 relative">
        {undoRequestedBy === playerName && (
          <div className={`absolute top-4 left-4 right-4 rounded-xl border p-3 z-10 text-center shadow-sm animate-pulse ${isDarkMode ? "bg-purple-900/30 border-purple-800 text-purple-300" : "bg-purple-50 border-purple-200 text-purple-600"}`}>
            <p className="text-sm font-medium">Đang chờ phản hồi xin đi lại...</p>
          </div>
        )}
        ...
        {activeTab === "players" && (
          <div className="flex flex-col gap-5 h-full">
            {playerName === hostName && !isGameStarted && (
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2">
                <h4 className={`text-sm font-bold transition-colors flex items-center justify-between ${isDarkMode ? "text-red-400" : "text-red-700"}`}>
                  Đội Đỏ (Đi trước)
                  {!isGameStarted && playerName !== hostName && <span className={`text-xs font-medium ${isDarkMode ? "text-slate-400" : "text-zinc-500"}`}>{Math.floor(initialTime / 60)} phút</span>}
                </h4>
                <div className={`flex items-center justify-between border p-2.5 rounded-lg transition-colors ${isDarkMode ? "bg-slate-700/30 border-slate-600" : "bg-white border-zinc-200 shadow-sm"}`}>
                  {player1Name ? (
                    <div className="flex items-center gap-2 overflow-hidden w-full">
                      ...
                      {playerName === hostName && player1Name !== hostName && (!isGameStarted || winner) && (
                  ...
                  <div className={`flex items-center justify-between border p-2.5 rounded-lg transition-colors ${isDarkMode ? "bg-slate-700/30 border-slate-600" : "bg-white border-zinc-200 shadow-sm"}`}>
                    {player3Name ? (
                      <div className="flex items-center gap-2 overflow-hidden w-full">
                        ...
                        {playerName === hostName && player3Name !== hostName && (!isGameStarted || winner) && (
              ...
              {/* Team Blue (Đi sau) */}
              <div className="flex flex-col gap-2 mt-2">
                <h4 className={`text-sm font-bold transition-colors flex items-center justify-between ${isDarkMode ? "text-blue-400" : "text-blue-700"}`}>
                  Đội Xanh (Đi sau)
                </h4>
                <div className={`flex items-center justify-between border p-2.5 rounded-lg transition-colors ${isDarkMode ? "bg-slate-700/30 border-slate-600" : "bg-white border-zinc-200 shadow-sm"}`}>
                  {player2Name ? (
                    <div className="flex items-center gap-2 overflow-hidden w-full">
                      <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold transition-colors shadow-sm ${isDarkMode ? "bg-slate-900 text-white border border-slate-700" : "bg-blue-50 border-blue-200 text-blue-800"}`}>
                      ...
                      {playerName === hostName && player2Name !== hostName && (!isGameStarted || winner) && (
                    ...
                    <button onClick={() => handleSlotClick(2)} className={`text-sm font-medium py-1 px-2 border border-dashed rounded w-full text-left transition-colors ${isDarkMode ? "text-slate-400 hover:text-slate-200 border-slate-600 hover:border-slate-400" : "text-zinc-500 hover:text-zinc-800 border-zinc-300 hover:border-zinc-500"}`}>
                      + Ngồi vào ghế 2
                    </button>
                  )}
                </div>
                  ...
                  <div className={`flex items-center justify-between border p-2.5 rounded-lg transition-colors ${isDarkMode ? "bg-slate-700/30 border-slate-600" : "bg-white border-zinc-200 shadow-sm"}`}>
                    {player4Name ? (
                      <div className="flex items-center gap-2 overflow-hidden w-full">
                        ...
                        {playerName === hostName && player4Name !== hostName && (!isGameStarted || winner) && (
                      ...
                      <button onClick={() => handleSlotClick(4)} className={`text-sm font-medium py-1 px-2 border border-dashed rounded w-full text-left transition-colors ${isDarkMode ? "text-slate-400 hover:text-slate-200 border-slate-600 hover:border-slate-400" : "text-zinc-500 hover:text-zinc-800 border-zinc-300 hover:border-zinc-500"}`}>
                        + Ngồi vào ghế 4
                      </button>
                    )}
                  </div>
              ...
              {!isSpectator && (!isGameStarted || winner) && (
                <button
                  onClick={handleBecomeSpectator}
                  className={`mt-4 text-sm underline text-center w-full transition-colors ${isDarkMode ? "text-slate-400 hover:text-slate-300" : "text-zinc-500 hover:text-zinc-700"}`}
                >
          ...
          <div className="h-full">
            {spectators.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full opacity-50 py-10">
                ...
                <p className={`text-sm font-medium ${isDarkMode ? "text-slate-300" : "text-zinc-600"}`}>Chưa có người xem</p>
              </div>
            ) : (
              <ul className="space-y-2">
                {spectators.map((spec, idx) => (
                  <li
                    key={idx}
                    className={`group flex items-center justify-between p-2.5 rounded-xl border transition-all ${isDarkMode ? "bg-slate-800/50 border-slate-700 hover:bg-slate-700" : "bg-zinc-50 border-zinc-100 hover:bg-zinc-100 hover:border-zinc-200"}`}
                  >
                    <div className="flex items-center space-x-3 overflow-hidden">
                      ...
                      <span className={`text-sm font-medium truncate ${isDarkMode ? "text-slate-200" : "text-zinc-800"}`} title={spec}>
                        {spec}
                      </span>
                    </div>
                    {playerName === hostName && (
                      <button
                        ...
                        className={`shrink-0 rounded-lg px-2.5 py-1.5 text-xs font-semibold opacity-0 transition-all group-hover:opacity-100 focus:opacity-100 ${isDarkMode ? "bg-red-900/30 text-red-400 hover:bg-red-900/50" : "bg-red-50 text-red-600 hover:bg-red-100"}`}
                      >
                        Kick
                      </button>
                    )}
